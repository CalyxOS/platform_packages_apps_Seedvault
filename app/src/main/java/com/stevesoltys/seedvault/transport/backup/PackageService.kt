...
internal fun PackageInfo.allowsBackup(): Boolean {
    if (packageName == MAGIC_PACKAGE_MANAGER || applicationInfo == null) return false
    return applicationInfo.targetSdkVersion >= Build.VERSION_CODES.O || // S
...
internal fun PackageInfo.doesNotGetBackedUp(): Boolean {
    if (packageName == MAGIC_PACKAGE_MANAGER || applicationInfo == null) return true
    return (applicationInfo.targetSdkVersion < Build.VERSION_CODES.O &&
        applicationInfo.flags and FLAG_ALLOW_BACKUP == 0) || // does not allow backup
